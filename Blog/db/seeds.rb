...
10.times do
  article = Article.create!(title: Faker::HarryPotter.unique.book, content: Faker::GreekPhilosophers.quote, user_id: (rand(User.first.id..User.last.id)), category_id: (rand(Category.first.id ..Category.last.id)))
end

# Création des 15 commentaires, écrits chacun par un utilisateur, et relié à un article
15.times do 
  comment = Comment.create!(content: Faker::SiliconValley.unique.quote, user_id: (rand(User.first.id..User.last.id)), article_id: (rand(Article.first.id..Article.last.id)))
end
